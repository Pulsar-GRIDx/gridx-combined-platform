...
const meter = require("./models/PostpaidBillingConfigModel");
...
// Route to get billing data by billing type
router.get('/getByBillingType/:billingType',auth, (req, res) => {
  const { billingType } = req.params;
  meter.getByBillingType(billingType, (err, data) => {
    if (err) {
      return res.status(500).json({ error: "Error fetching billing data", message: err.message });
    }
    return res.json(data);
  });
});

router.post("/postPaidCreateByDRN/:id", auth, function (req, res) {
  // Validate request
  if (!req.body) {
