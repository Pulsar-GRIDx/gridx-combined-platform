...
const meter = require("../models/LoadControl/meterMainsControlModel");
...
router.post("/update/:id", auth, async function (req, res) {
  ...
  // Send mains control command via MQTT for immediate delivery
  try {
    mqttHandler.publishCommand(drn, { mc: state }, 1);
    console.log(`[MainsControl] MQTT command sent to ${drn}: state=${state}`);
  } catch (e) {
    console.error(`[MainsControl] MQTT publish failed for ${drn}:`, e.message);
  }
