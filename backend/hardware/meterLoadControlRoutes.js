...
const meter = require("./models/meterLoadControlModel");
...
router.post('/addRandomSampleData', async (req, res) => {
  ...
  function generateRandomSampleData() {
    return [
      Math.random() < 0.5 ? 0 : 1,    // Random active_energy (between 0 and 100)
      Math.random() < 0.5 ? 0 : 1,       // Random reactive_energy (between 0 and 10)
      Math.random() < 0.5 ? 0 : 1,      // Random units (between 0 and 1000)
...
router.get("/findById/:id", auth, function (req, res) {
  meter.findById(req.params.id, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
...
router.get("/getByDRNLastItem", auth, function (req, res) {
  meter.getLastUpdate(req.DRN, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Not found meter with DRN ${req.params.DRN}.`,
