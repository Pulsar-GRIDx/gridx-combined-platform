...
Load.create = (data, result) => {
  connection.query(
    "INSERT INTO MeterHeaterControlTable SET ?",
    data,
    (err, res) => {
      if (err) {
        result(err, null);
...
Load.getAll = (DRN, result) => {
  let query = "SELECT * FROM MeterHeaterControlTable";
...
Load.findById = (DRN, result) => {
  connection.query(
    `SELECT * FROM MeterHeaterControlTable WHERE DRN = ${DRN}`,
    (err, res) => {
      if (err) {
        result(err, null);
        return;
...
Load.remove = (DRN, result) => {
  connection.query(
    "DELETE FROM MeterHeaterControlTable WHERE DRN = ?",
    DRN,
    (err, res) => {
      if (err) {
        result(null, err);
...
Load.removeAll = (result) => {
  connection.query("DELETE FROM MeterHeaterControlTable", (err, res) => {
    if (err) {
      result(null, err);
      return;
    }
...
Load.getLastCalibrationUpdate= (DRN, result) => {
  const query = "SELECT * FROM MeterHeaterControlTable WHERE ID = (SELECT MAX(ID) FROM MeterHeaterControlTable WHERE DRN = ?)";
...
Load.updatePrecessed = (DRN, processed,ID,result) => {
  connection.query(
    "UPDATE MeterHeaterControlTable SET processed = ? WHERE DRN = ? AND ID = ?",
    [processed,DRN,ID],
    (err, res) => {
      if (err) {
        result(null, err);
