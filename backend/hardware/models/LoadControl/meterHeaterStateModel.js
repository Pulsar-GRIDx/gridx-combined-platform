...
Load.create = (data, result) => {
  connection.query(
    "INSERT INTO MeterHeaterStateTable SET ?",
    data,
    (err, res) => {
      if (err) {
        result(err, null);
...
Load.getAll = (DRN, result) => {
  let query = "SELECT * FROM MeterHeaterStateTable";
...
Load.findById = (DRN, result) => {
  connection.query(
    `SELECT * FROM MeterHeaterStateTable WHERE DRN = ${DRN}`,
    (err, res) => {
      if (err) {
        result(err, null);
        return;
...
Load.remove = (DRN, result) => {
  connection.query(
    "DELETE FROM MeterHeaterStateTable WHERE DRN = ?",
    DRN,
    (err, res) => {
      if (err) {
        result(null, err);
...
Load.removeAll = (result) => {
  connection.query("DELETE FROM MeterHeaterStateTable", (err, res) => {
    if (err) {
      result(null, err);
      return;
    }
...
Load.getLastCalibrationUpdate= (DRN, result) => {
  const query = "SELECT * FROM MeterHeaterStateTable WHERE ID = (SELECT MAX(ID) FROM MeterHeaterStateTable WHERE DRN = ?)";
...
Load.updatePrecessed = (DRN, processed,ID,result) => {
  connection.query(
    "UPDATE MeterHeaterStateTable SET processed = ? WHERE DRN = ? AND ID = ?",
    [processed,DRN,ID],
    (err, res) => {
      if (err) {
        result(null, err);
