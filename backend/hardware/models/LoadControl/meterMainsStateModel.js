...
Load.create = (data, result) => {
  connection.query(
    "INSERT INTO MeterMainsStateTable SET ?",
    data,
    (err, res) => {
      if (err) {
        result(err, null);
...
Load.getAll = (DRN, result) => {
  let query = "SELECT * FROM MeterMainsStateTable";
...
Load.findById = (DRN, result) => {
  connection.query(
    `SELECT * FROM MeterMainsStateTable WHERE DRN = ${DRN}`,
    (err, res) => {
      if (err) {
        result(err, null);
        return;
...
Load.remove = (DRN, result) => {
  connection.query(
    "DELETE FROM MeterMainsStateTable WHERE DRN = ?",
    DRN,
    (err, res) => {
      if (err) {
        result(null, err);
...
Load.removeAll = (result) => {
  connection.query("DELETE FROM MeterMainsStateTable", (err, res) => {
    if (err) {
      result(null, err);
      return;
    }
...
Load.getLastCalibrationUpdate= (DRN, result) => {
  const query = "SELECT * FROM MeterMainsStateTable WHERE ID = (SELECT MAX(ID) FROM MeterMainsStateTable WHERE DRN = ?)";
...
Load.updatePrecessed = (DRN, processed,ID,result) => {
  connection.query(
    "UPDATE MeterMainsStateTable SET processed = ? WHERE DRN = ? AND ID = ?",
    [processed,DRN,ID],
    (err, res) => {
      if (err) {
        result(null, err);
