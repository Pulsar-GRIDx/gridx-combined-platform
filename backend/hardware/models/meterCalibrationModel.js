...
Calibration.findById = (DRN, result) => {
  connection.query(
    `SELECT * FROM Calibration WHERE DRN = ${DRN}`,
    (err, res) => {
      if (err) {
        result(err, null);
        return;
...
Calibration.remove = (DRN, result) => {
  connection.query(
    "DELETE FROM Calibration WHERE DRN = ?",
    DRN,
    (err, res) => {
      if (err) {
        result(null, err);
...
Calibration.removeAll = (result) => {
  connection.query("DELETE FROM Calibration", (err, res) => {
    if (err) {
      result(null, err);
      return;
    }
...
Calibration.getLastCalibrationUpdate= (DRN, result) => {
  const query = "SELECT * FROM Calibration WHERE ID = (SELECT MAX(ID) FROM Calibration WHERE DRN = ?)";
...
Calibration.updatePrecessed = (DRN, processed,ID,result) => {
  connection.query(
    "UPDATE Calibration SET processed = ? WHERE DRN = ? AND ID = ?",
    [processed,DRN,ID],
    (err, res) => {
      if (err) {
        result(null, err);
