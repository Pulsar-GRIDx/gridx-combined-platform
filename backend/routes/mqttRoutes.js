...
async function _refreshDashCache(thresholdSec = 300) {
  const [
    ...
    queryAll(`SELECT s.id, s.DRN, s.token_id, s.token_cls, s.submission_Method, s.display_msg, s.display_auth_result, s.display_token_result, s.display_validation_result, s.token_time, s.token_amount, s.date_time, COALESCE(tg.flatRate, 2.45) as tariff_rate FROM STSTokesInfo s LEFT JOIN MeterProfileReal mpr ON s.DRN = mpr.DRN LEFT JOIN TariffGroups tg ON mpr.tariff_type = tg.name ORDER BY s.id DESC LIMIT 30`, []),
    queryAll(`SELECT HOUR(date_time) as hour, COUNT(*) as count, ROUND(COALESCE(SUM(token_amount), 0), 2) as revenue FROM STSTokesInfo WHERE DATE(date_time) = CURDATE() AND display_msg LIKE '%Accept%' GROUP BY HOUR(date_time) ORDER BY hour`, []),
    ...
    queryAll(`SELECT HOUR(date_time) as hour, ROUND(COALESCE(SUM(delta) / 1000, 0), 3) as kWh FROM (SELECT DRN, HOUR(date_time) as hour, CAST(MAX(active_energy) AS DECIMAL(12,2)) - CAST(MIN(active_energy) AS DECIMAL(12,2)) as delta FROM MeterCumulativeEnergyUsage WHERE DATE(date_time) = CURDATE() GROUP BY DRN, HOUR(date_time) HAVING delta > 0) h GROUP BY hour ORDER BY hour`, []),
